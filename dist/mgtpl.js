...
var lang$1 = {
  'cn': (_cn = {
    ...
    'scale': '缩放比例',
    'output': '输出大小'
  }, _defineProperty(_cn, "export", '导出'), _defineProperty(_cn, 'print', '打印'), _defineProperty(_cn, 'error', '图幅太大，导出时可能出现内存不足'), _cn),
  'en': (_en = {
    ...
    'scale': 'Scale',
    'output': 'Output Size'
  }, _defineProperty(_en, "export", 'Export'), _defineProperty(_en, 'print', 'Print'), _defineProperty(_en, 'error', 'Image size is too large, the export may appear memory error'), _en)
};

var language = void 0;

var template = '\
  <h3 style="text-align: center;">' + lang$1[language]['title'] + '</h3>\
  <div>\
  <label>' + lang$1[language]['size'] + '</label>\
  <span class ="graph-export-panel__canvas_size"></span>\
  </div>\
  <div style="text-align: center;" title="' + lang$1[language]['doubleClick'] + '">\
  <div class ="graph-export-panel__export_canvas" style="position: relative; display: inline-block;">\
  </div>\
  </div>\
  <div>\
  <label>' + lang$1[language]['export'] + '</label>\
  <span class ="graph-export-panel__export_bounds"></span>\
  </div>\
  <div>\
  <label>' + lang$1[language]['scale'] + ': <input class ="graph-export-panel__export_scale" type="range" value="1" step="0.2" min="0.2" max="3"><span class ="graph-export-panel__export_scale_label">1</span></label>\
  </div>\
  <div>\
  <label>' + lang$1[language]['output'] + ': </label><span class ="graph-export-panel__export_size"></span>\
  </div>\
  <div style="text-align: right">\
  <button type="submit" class="btn btn-primary graph-export-panel__export_submit">' + lang$1[language]['export'] + '</button>\
  <button type="submit" class="btn btn-primary graph-export-panel__print_submit">' + lang$1[language]['print'] + '</button>\
  </div>';
  ...
  _createClass$1(ExportPanel, [{
    ...
    value: function initCanvas() {
      ...
      this.updateOutputSize = function () {
        ...
        var info = w + " X " + h;
        if (w * h > 3000 * 4000) {
          info += "<span style='color: #F66;'>" + lang$1[language]['error'] + "</span>";
        }
...
function showExportPanel(graph, langs) {
  // 显示参数 - 适配中英文
  language = langs;
  if (!exportPanel) {
